...
const AdminDashboardDetails=async (req:Request,res:Response)=>{
    try {
        // pending reviews
        const reviewAggregation=(await Review.find({reviewStatus:false})).length
        // completed reviews
        const completedReviewAggregation=(await Review.find({reviewStatus:true})).length
        ...
        const paymentsuccessAggregation=(await Payment.find({status:true})).length
        // cumulative failure amount
        const paymentfailureAggregation=(await Payment.find({status:false})).length
        // user count
