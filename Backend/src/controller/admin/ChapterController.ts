...
const AddChapter = async (req: Request, res: Response) => {
  try {
    ...
    if (
      spaceValidator(roadmapId) ||
      spaceValidator(chapterName) ||
      spaceValidator(additionalPrompt)
    ) {
      return res.status(400).json({ message: 'bad request' });
    }
    if (
      minLengthValidator(roadmapId) ||
      minLengthValidator(chapterName) ||
      minLengthValidator(additionalPrompt)
    ) {
      return res.status(400).json({ message: 'bad request' });
    }
    ...
    addMessageToQueue(
      ...
      JSON.stringify({
        ...
        roadmapId: savedChapter.roadmapId,
      })
    );
  ...
  try {
    ...
    if (user.authorization !== 'admin') {
      ...
      if (UpdateChapter) {
        ...
        UpdateChapter.additionalPrompt =
          additionalPrompt ?? UpdateChapter.additionalPrompt;
