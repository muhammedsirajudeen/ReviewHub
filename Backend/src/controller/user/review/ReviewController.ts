...
const GetReview=async (req:Request,res:Response)=>{
    try{
        ...
        const userReviews=await Review.find({revieweeId:user.id,scheduledDate:{$exists:true}})
...
const RequestReview=async (req:Request,res:Response)=>{
    try{
        ...
        const checkReview=await Review.findOne({revieweeId:user.id,roadmapId:roadmapId})
        if(checkReview){
            return res.status(409).json({message:"review already requested"})
        }
        //second check
        const checkCount=(await Review.find({revieweeId:user.id,reviewStatus:false})).length
        if(checkCount>3){
...
const GetScheduledRoadmaps=async (req:Request,res:Response)=>{
    try{
        ...
        const scheduledRoadmaps=await Review.find({revieweeId:user.id,scheduledDate:{$exists:false}}).populate('roadmapId')
        console.log(scheduledRoadmaps)
...
const ScheduleReview=(req:Request,res:Response)=>{
    try{
