...
export default function Approval(): ReactElement {
  ...
  const onSubmit: SubmitHandler<ReviewerApprovalFormData> = async (data) => {
    ...
    if(!pdfFile){
      ...
      if(data.comments) formData.append("comment",data.comments)
        ...
        const response=(
          await axiosInstance.post('/reviewer/approval',
            formData,
            {
              headers:{
  ...
  return (
    <div className="p-6 max-w-md mx-auto bg-white rounded-lg shadow-md mt-40">
      <h2 className="text-3xl font-semibold mb-6 text-center text-gray-800">Reviewer Approval Form</h2>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mb-5">
          ...
          <input
            ...
            className={`mt-1 block w-full p-3 border rounded-md transition duration-200 ${errors.experience ? 'border-red-500' : 'border-gray-300'}`}
            type="number"
            id="experience"
          />
        ...
        <div className="mb-5">
          ...
          <textarea
            ...
            className={`mt-1 block w-full p-3 border rounded-md transition duration-200 ${errors.comments ? 'border-red-500' : 'border-gray-300'}`}
            id="comments"
          />
        ...
        {fileName && <p className="text-gray-700 text-sm">Uploaded File: {fileName}</p>}
