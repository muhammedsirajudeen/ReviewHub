...
export default function WithdrawalDialog({
  ...
  return (
    <dialog
      ...
      className="relative h-96 w-96 rounded-lg p-6 bg-white shadow-lg"
      ref={dialogRef}
    >
      <h1 className="w-full text-center mt-4 text-xl font-semibold text-gray-800">
        Withdrawal
      </h1>
      ...
      {user &&
        user.paymentMethod.map((paymentMethod) => {
          return (
            <div
        ...
        <input
          ...
          {...register('amount', {
            ...
            validate: {
              noSpaces: (value) =>
                value.trim().length > 0 ||
                'Amount cannot be empty or only spaces',
            },
          })}
        />
        {errors.amount && (
