...
export default function ChapterForm({
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    try {
      ...
      if (response.data.message === 'success') {
        toast.success(
          `${method === 'post' ? 'Created' : 'Edited'} successfully`
        );
        if (method === 'post') {
          setChapters(
            produce((draft) => {
  ...
  return (
    <dialog
      ...
      ref={dialogRef}
      className="flex flex-col items-center justify-start w-96  p-4 bg-white rounded-lg shadow-md"
    >
      ...
      <form className="flex flex-col w-full" onSubmit={handleSubmit(onSubmit)}>
        <label htmlFor="chapterName" className="text-sm font-medium mt-4">
          Chapter Name
        </label>
        <input
          id="chapterName"
          ...
          {...register('chapterName', {
            ...
            validate: (tag: string) => {
              ...
              if (SpecialCharRegex.test(tag))
                return 'Invalid characters in chapter name';
              return true;
            },
          })}
        />
        ...
        <label htmlFor="additionalPrompt" className="text-sm font-medium mt-4">
          Additional Prompt
        </label>
        <textarea
          id="additionalPrompt"
          ...
          {...register('additionalPrompt', {
            ...
            validate: (tag: string) => {
              if (tag.trim() === '')
                return 'Please enter a valid additional prompt';
              if (SpecialCharRegex.test(tag))
                return 'Invalid characters in additional prompt';
              return true;
            },
          })}
        />
        ...
        {method === 'put' && (
          <Link
            ...
            state={{
              ...
              roadmapId,
              quizStatus: chapter?.quizStatus,
            }}
            className="p-2 text-sm border border-black mt-4 text-center hover:bg-gray-100 rounded"
          >
            View Chapter
          </Link>
        )}
        ...
        <button
          className="mt-4 border border-black p-2 text-sm rounded bg-blue-500 text-white hover:bg-blue-600"
          type="submit"
        >
          Submit
        </button>
