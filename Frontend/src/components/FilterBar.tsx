...
export default function FilterBar({
  ...
  useEffect(() => {
    ...
    if (selectdate && domain && favorite) {
      fetchData(
        `/user/course?page=${currentpage}&date=${date}&domain=${domain}&favorite=true`
      );
    } else if (domain) {
  ...
  return (
    <>
      <div className="flex items-center justify-evenly mt-4 px-8">
        ...
        <div className="relative flex flex-col items-center">
          <button
            className={`border border-gray-300 text-gray-700 p-2 rounded-lg flex items-center justify-center transition-colors duration-300 hover:bg-gray-100 ${
              active === 'domain' ? 'bg-gray-800 text-white' : ''
            }`}
        ...
        <button
          className={`border border-gray-300 text-gray-700 p-2 rounded-lg flex items-center justify-center transition-colors duration-300 hover:bg-gray-100 ${
            active === 'favorite' ? 'bg-gray-800 text-white' : ''
          }`}
          onClick={() => {
            toggleActive('favorite');
            favoriteSelector();
          }}
        >
